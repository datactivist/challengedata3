...
Dans le cadre de ce challenge data, notre plus gros casse-tête a été de savoir utiliser les logiciels et outils techniques. Rappelons nous par exemple toutes les formules Excel nécessaires pour créer le jeu de données des prénoms des nouveaux-nés, couplé au\_ join \_nécessaire pour récupérer le sexe à partir d’un autre fichier récupéré sur data.gouv. Rappelons nous également la datavisualisation de la carte de la provenance géographique des enfants scolarisés à Neuville Saint Rémy, ou bien encore la datavisualisation dynamique du palmarès des prénoms… un vrai casse-tête.
